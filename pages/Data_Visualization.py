...
if uploaded_file:
    ...
    #柱状图1
    def build1(df):
        x = df['薪资'].value_counts()[0:20].keys()
        y = df['薪资'].value_counts()[0:20].values
        fig, ax = plt.subplots()
        ax.bar(x, y)
        plt.xlabel('薪资范围')
        plt.ylabel('出现次数')
        plt.title("薪资分布前20柱状图")
        return fig
    ...
    def sandian1(df):
        ...
        y = df['薪资'].value_counts()[0:20].values
        fig, ax = plt.subplots()
        ax.scatter(x, y, color="red", label=u"薪资分布数据", linewidth=2)
        plt.xlabel('薪资范围')
        plt.ylabel('出现次数')
        plt.title("薪资分布前20散点图")
        plt.legend()
        return fig
    ...
    def zhexian1(df):
        ...
        y = df['薪资'].value_counts()[0:20].values
        fig, ax = plt.subplots()
        ax.plot(x, y, marker='o', color='b', label="薪资分布数据")
        plt.xlabel('薪资范围')
        plt.ylabel('出现次数')
        plt.title("薪资分布前20折线图")
        plt.legend()
        return fig
